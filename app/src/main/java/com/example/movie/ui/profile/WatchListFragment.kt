...
import com.example.movie.databinding.ProfileWatchListFragmentBinding
...
@AndroidEntryPoint
class WatchListFragment : Fragment(R.layout.profile_watch_list_fragment) {

    @Inject
    lateinit var glideRequestManager: RequestManager

    private val binding by viewBinding(ProfileWatchListFragmentBinding::bind)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            watchListFilmSwipeRefresh.setColorSchemeResources(R.color.colorPrimary)
            watchListFilmSwipeRefresh.setOnRefreshListener { fragmentViewModel.updateWatchList() }
        }
    }
    ...
    private fun fillRecyclerView() {
        ...
        with(binding) {
            if (resources.configuration.orientation ==
                ...
                rvWatchList.layoutManager = GridLayoutManager(context, 2)
            } else {
                rvWatchList.layoutManager = GridLayoutManager(context, 4)
            }

            rvWatchList.itemAnimator = DefaultItemAnimator()
            rvWatchList.adapter = filmAdapter
            filmAdapter.notifyDataSetChanged()
            watchListFilmSwipeRefresh.isRefreshing = false
        }
    }
